...
class MainActivity : AppCompatActivity(), GeoLocationHelper.LastLocationCallback {
    ...
    private fun observeLocationUpdates() {
        ...
        locationCallback = object : LocationCallback() {
            ...
            override fun onLocationResult(locationResult: LocationResult) {
                ...
                locationResult.lastLocation?.let { viewModel.saveCurrentLocation(it) }
            }
        }

        if (viewModel.checkPermissions())
            if (isLocationEnabled())
                geoLocationHelper.startLocationTracking(locationCallback, this)
            else
                openSettings()
        else
    ...
    override fun onDestroy() {
        super.onDestroy()
        geoLocationHelper.stopLocationTracking()
    }

    override fun lastLocationCallback(location: Location) {
        location.let { viewModel.saveCurrentLocation(it) }
    }
}